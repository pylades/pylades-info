...
function init() {
  ...
  try {
    const parsed = JSON.parse(window.localStorage.getItem(PYLADES))
...
export function Updater() {
  ...
  useEffect(() => {
    window.localStorage.setItem(PYLADES, JSON.stringify({ ...state, [LAST_SAVED]: Math.floor(Date.now() / 1000) }))
  })
