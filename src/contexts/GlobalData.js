...
const UPDATE_ALL_PAIRS_IN_PYLADES = 'UPDAUPDATE_ALL_PAIRS_IN_PYLADESTE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_PYLADES = 'UPDATE_ALL_TOKENS_IN_PYLADES'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_PYLADES: {
    ...
    case UPDATE_ALL_TOKENS_IN_PYLADES: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInPylades = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_PYLADES,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInPylades = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_PYLADES,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInPylades,
            updateAllTokensInPylades,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.pyladesFactories[0]
    ...
    oneDayData = oneDayResult.data.pyladesFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.pyladesFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.pyladesFactories[0]

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(twoWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const twoWeekData = twoWeekResult.data.pyladesFactories[0]
...
const getChartData = async (oldestDateToFetch) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.pyladesDayDatas)
      if (result.data.pyladesDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on pylades, used for search
 */
async function getAllPairsOnPylades() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on pylades, used for search
 */
async function getAllTokensOnPylades() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInPylades, updateAllTokensInPylades }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnPylades()
      updateAllPairsInPylades(allPairs)

      let allTokens = await getAllTokensOnPylades()
      updateAllTokensInPylades(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInPylades, updateAllTokensInPylades])
...
export function useAllPairsInPylades() {
...
export function useAllTokensInPylades() {
  const [state] = useGlobalDataContext()
